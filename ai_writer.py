...
def generate_engagement_tweet(
    ...
    prompt = (
        ...
        f"Format for Twitter readability:\n"
        f"- Use short lines (max 1 sentence per line)\n"
        f"- Add a blank line between each idea\n"
        f"- NO paragraph blocks under any circumstance\n\n"
        f"MANDATORY:\n"
        f"- Include at least one actionable price trigger using this structure:\n"
        f"  'Break above $X → continuation / move higher'\n"
        f"  'Lose $X → downside move / liquidity sweep'\n"
        f"- The tweet MUST tell the reader what to watch next\n"
        f"- Avoid vague statements like 'watch the reaction' or "
        f"'market is compressing' without levels\n"
        f"- Output must be scannable in under 2 seconds and feel actionable\n\n"
        f"Rules:\n"
